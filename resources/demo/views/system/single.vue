...
import {SingleList, SingleDestroy, SingleUpdate} from '../../utils/request.js';
...
export default {
    ...
    methods: {
        async list() {
            await this.$func.verify('single.list');
            this.loading = true;
            SingleList(this.params).then((res) => {
                if (res.status) {
        ...
        async create() {
            await this.$func.verify('single.update');
            this.form = {
                id: 0,
                title: '',
                content: '',
            };
            this.dialog.title = '新增单页';
        ...
        async destroy(id) {
            await this.$func.verify('single.destroy');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                SingleDestroy({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.list();
