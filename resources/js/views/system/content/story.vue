            ...
            <el-form v-if="form" :model="form" :rules="rules" ref="form" class="board-form" label-width="120px">
                ...
                <div class="form-item">
                    ...
                    <el-form-item label="内 容：" prop="content">
                        <editor v-model="form.content" :height="350"></editor>
                    </el-form-item>
...
export default {
    ...
    methods: {
        ...
        async list() {
            await this.$func.verify('story.list');
            this.loading = true;
            StoryList(this.params).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.datas = res.data;
        ...
        async create() {
            await this.$func.verify('story.update');
            this.form = {
                id: 0,
                title: '',
                content: '',
        ...
        update() {
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    ...
                    StoryUpdate(this.form).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
        ...
        async active(){
            await this.$func.verify('story.active');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            StoryActive({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async good(){
            await this.$func.verify('story.good');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            StoryGood({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async lock(){
            await this.$func.verify('story.lock');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            StoryLock({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async destroy(id) {
            await this.$func.verify('story.destroy');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                StoryDestroy({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.list();
