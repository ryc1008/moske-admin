...
<!--                    https://m.media-amazon.com/images/I/41sZ2olMC3L._SL500_.jpg-->
...
export default {
    ...
    methods: {
        ...
        async list() {
            await this.$func.verify('novel.list');
            this.loading = true;
            NovelList(this.params).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.datas = res.data;
        ...
        async create() {
            await this.$func.verify('novel.update');
            this.form = {
                id: 0,
                title: '',
                blurb: '',
        ...
        update() {
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    ...
                    NovelUpdate(this.form).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
        ...
        async active(){
            await this.$func.verify('novel.active');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            NovelActive({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async lock(){
            await this.$func.verify('novel.lock');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            NovelLock({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async destroy(id) {
            await this.$func.verify('novel.destroy');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                NovelDestroy({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.list();
        ...
        async chapter(id) {
            await this.$func.verify('novel.chapter');
            NovelChapter({id: id}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
        ...
        editChapter(detail) {
            ...
            this.novelId = detail.novel_id;
            this.$nextTick(()=>{
                this.$refs['formChapter'].clearValidate();
            });
        },
        async updateChapter() {
            await this.$func.verify('novel.save');
            this.$refs['formChapter'].validate((valid) => {
                if (valid) {
                    this.button.disabled = true;
                    this.button.loading = true;
                    this.button.text = '提交中...';
                    NovelSave(this.formChapter).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
                            this.$func.success(res.message).then(() => {
                                this.chapter(this.novelId);
                            })
                        }
                        this.button.disabled = false;
                        this.button.loading = false;
        ...
        async destroyChapter(id) {
            await this.$func.verify('novel.delete');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                NovelDelete({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.chapter(this.novelId);
