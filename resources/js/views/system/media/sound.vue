<template>
    <div class="board-content">
            ...
            <el-table v-if="datas" element-loading-text="努力加载中..."
                ...
                <el-table-column label="操作" fixed="right" min-width="260">
                    <template slot-scope="scope">
                        ...
                        <el-button type="warning" icon="fa fa-sitemap" size="mini" plain  @click="chapter(scope.row.id, scope.row.thumb)"> 章节</el-button>
        ...
        <el-dialog class="board-dialog dialog-chapter"  fullscreen :close-on-click-modal="false"
                   :close-on-press-escape="false"
                   :title="dialogChapter.title" @close="closeDialog"
                    ...
                    <el-form v-if="formChapter" :model="formChapter" :rules="ruleChapters" ref="formChapter" class="board-form" label-width="120px">
                        ...
                        <div class="form-item">
                            ...
                            <el-form-item label="资 源：" prop="target">
                                <el-input v-model="formChapter.target" placeholder="请填写资源地址" prefix-icon="fa fa-link" size="medium" clearable></el-input>
                            </el-form-item>
                            <el-form-item v-if="formChapter.target" class="player">
                                <music ref="music" :url="formChapter.target" :poster="soundPoster" @time="time"></music>
                            </el-form-item>
                            <div style="text-align: center">
                                <el-button type="primary" :loading="button.loading" :disabled="button.disable"
                                           @click="updateChapter">{{ button.text }}</el-button>
                                <el-button v-if="formChapter.id" style="margin-left: 50px" type="info" @click="destroyChapter(formChapter.id)">删除</el-button>
                            </div>
                        </div>
                    </el-form>
                </div>
...
export default {
    ...
    methods: {
        ...
        async list() {
            await this.$func.verify('sound.list');
            this.loading = true;
            SoundList(this.params).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.datas = res.data;
        ...
        async create() {
            await this.$func.verify('sound.update');
            this.form = {
                id: 0,
                title: '',
                blurb: '',
        ...
        update() {
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    ...
                    SoundUpdate(this.form).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
        ...
        async active(){
            await this.$func.verify('sound.active');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            SoundActive({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async lock(){
            await this.$func.verify('sound.lock');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            SoundLock({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async good(){
            await this.$func.verify('sound.good');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            SoundGood({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async destroy(id) {
            await this.$func.verify('sound.destroy');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                SoundDestroy({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.list();
        ...
        async chapter(id, poster) {
            await this.$func.verify('sound.chapter');
            SoundChapter({id: id}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
        ...
        editChapter(detail) {
            this.formChapter = {
                id: detail.id,
                sound_id: detail.sound_id,
                title: detail.title,
                target: detail.target,
            };
            this.soundId = detail.sound_id;
            this.$nextTick(()=>{
                this.$refs['formChapter'].clearValidate();
            });
        },
        async updateChapter() {
            await this.$func.verify('sound.save');
            this.$refs['formChapter'].validate((valid) => {
                if (valid) {
                    this.button.disabled = true;
                    this.button.loading = true;
                    this.button.text = '提交中...';
                    SoundSave(this.formChapter).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
                            this.$func.success(res.message).then(() => {
                                this.chapter(this.soundId, this.soundPoster);
                            })
                        }
                        this.button.disabled = false;
                        this.button.loading = false;
        ...
        async destroyChapter(id) {
            await this.$func.verify('sound.delete');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                SoundDelete({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.chapter(this.soundId, this.soundPoster);
