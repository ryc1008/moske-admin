    ...
    <div class="board-content">
            ...
            <div class="board-search">
                ...
                <div class="search-item" v-if="configs">
                    <el-select v-model="params.type_id" style="width: 120px" filterable size="small">
                        <el-option :value="0" label="全部作者" ></el-option>
                        <el-option
        ...
        <div class="board-main">
            <el-table v-if="datas" element-loading-text="努力加载中..."
                ...
                <el-table-column prop="favor" label="收藏" width="80" align="center" sortable="custom"></el-table-column>
        ...
        <el-dialog class="board-dialog"  fullscreen :close-on-click-modal="false"
            ...
            <el-form v-if="form" :model="form" :rules="rules" ref="form" class="board-form" label-width="120px">
                ...
                <div class="form-item">
                    <el-form-item label="作 者：" prop="type_id" required>
                        <el-select v-model="form.type_id" size="medium">
                            <el-option :value="0" label="选择作者" ></el-option>
                            <el-option
                                v-for="(type,i) in configs.types"
                                :key="parseInt(i)"
                                :label="type"
...
export default {
    ...
    methods: {
        ...
        async list() {
            await this.$func.verify('playlet.list');
            this.loading = true;
            PlayletList(this.params).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.datas = res.data;
        ...
        async create() {
            await this.$func.verify('playlet.update');
            this.form = {
                id: 0,
                title: '',
                target: '',
        ...
        update() {
            this.$refs['form'].validate((valid) => {
                if (valid) {
                    ...
                    PlayletUpdate(this.form).then((res) => {
                        if (res.status) {
                            this.$func.error(res.message);
                        } else {
        ...
        async active(){
            await this.$func.verify('playlet.active');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            PlayletActive({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async lock(){
            await this.$func.verify('playlet.lock');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            PlayletLock({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async good(){
            await this.$func.verify('playlet.good');
            if(!this.selection.length){
                this.$func.error('请选择你要操作的数据'); return false;
            }
            PlayletGood({id: this.selection}).then((res) => {
                if (res.status) {
                    this.$func.error(res.message);
                } else {
                    this.list();
        ...
        async destroy(id) {
            await this.$func.verify('playlet.destroy');
            this.$confirm('您确定要删除选中的该条信息吗？', '提示', {
                type: 'warning',
                confirmButtonText: '确 定',
                cancelButtonText: '取 消'
            }).then(() => {
                PlayletDestroy({id: id}).then((res) => {
                    if (res.status) {
                        this.$func.error(res.message);
                    } else {
                        this.list();
        ...
        random(){
            ...
            this.form.hits = this.$func.randNumber(150, 1000);
